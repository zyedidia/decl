...
meterupdate(uint8_t* code, size_t size, size_t addr)
{
    ...
    while (count < size) {
        ...
        if (FD_TYPE(&instr) == FDI_LEA && FD_OP_REG(&instr, 0) == FD_REG_R12) {
            assert(ret == 8);
            int32_t amt = 0;
            memcpy(&code[count + 4], (char*) &amt, 4);
        }
        if (FD_TYPE(&instr) == FDI_SUB && FD_OP_REG(&instr, 0) == FD_REG_R12) {
            if (ret == 7) {
                int32_t amt = 0;
                memcpy(&code[count + 3], (char*) &amt, 4);
            }
        }
        bool indirect, cond;
        int64_t target;
        ...
        uint8_t nop16[16] = {0x66, 0x2e, 0x0f, 0x1f, 0x84, 0x00, 0x00, 0x00, 0x00, 0x00, 0x66, 0x0f, 0x1f, 0x44, 0x00, 0x00};
        if (branch) {
            size_t noploc = 3;
            ...
            if (count >= noploc &&
                ...
                if (target <= addr+count) {
                    ...
                    // nop out the shrx stuff
                    memcpy(&code[count - 16], nop16, 16);
                }
            }
        }
        count += ret;
