...
meteropt(uint8_t* buf, size_t sz, size_t addr)
{
    ...
    for (size_t i = 0; i < n; i++) {
        if (csi) {
            prev_id = csi->id;
            if (prev_id == ARM64_INS_LDR)
                prev_reg = csi->detail->arm64.operands[0].reg;
        }
        ...
        case ARM64_INS_RET:
            ...
            if (csi->id == ARM64_INS_BLR && csi->detail->arm64.operands[0].reg == ARM64_REG_X24 && prev_id == ARM64_INS_LDR && prev_reg == ARM64_REG_X24) {
                cs_free(csi, 1);
                continue;
            }
            break;
